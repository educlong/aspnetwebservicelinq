 ...
 * 
 * ĐÂY LÀ PHÍA CLIENT (ko tương tác trực tiếp vs LINQ mà tương tác vs các file trong controller(DanhMucController, SanPhamController))
 */
...
namespace GUIWebApplicationDemo.Views
{
    public partial class WebFormRestful : System.Web.UI.Page
    {
        ...
        {   /*vì chắc chắn trả về 1 mảng các danh mục nên truyền vào là typeof(DanhMucNoLinq[]) và trả về []*/
        ...
        {
            try/*trước khi xóa danh mục, phải check xem trong danh mục còn sản phẩm ko, nếu còn thì ko đc xóa, nếu ko còn thì mới xóa*/
            {  /*Việc check này đã đc xử lỷ trong DataAccessLayer rồi, nên ở đây ko cần check lại nữa*/
                lblQueryDM.Text = ((bool)ArrayObjects(url + urlDanhMuc + string.Format("?madm={0}", txtCodeDm.Text),
                                                      checkDanMucExist() ? "DELETE" : "", 
                                                      typeof(bool))) 
                                    ? "Success" : "Fail";
            }
            catch (Exception ex)
